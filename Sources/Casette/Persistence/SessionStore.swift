...
//
// LIFTED from v0/10-persistence/Sources/SessionStore/SessionStore.swift (the
// V0.10 recovery engine, written to migrate verbatim — SESSION-FORMAT.md).
// The Codable types it persists were lifted in V1.2 and have since gained
// three ADDITIVE, omitted-when-nil fields (`truncation` V1.5,
// `PersistedArtifact.error` V1.7, `SessionRow.numeric` V1.8), so the schema
// version stays 1 and this store needed no schema changes.
//
// The ONE deliberate deviation from the v0/10 source (PROBLEMS.md "Parallel
// swift-testing + setenv"): `defaultSessionsDirectory` takes the environment
// as a PARAMETER (defaulting to the process environment at the production
// call site) so tests inject the override instead of racing `setenv`.
//
// Responsibilities:
//   * pretty-printed, sorted-keys, human-inspectable JSON (the spec demands
...
//   * INCREMENTAL save (rewrite after every appended/completed row);
//   * robust LOAD: corrupted JSON → quarantine the bad file + start fresh;
//     unknown schema version → polite refusal (file left intact); empty or
//     missing → fresh session.
...
public struct SessionStore {
  ...
  /// `CASETTE_CONFIG_DIR` to keep tests hermetic (consistent with v0/09).
  ...
  /// `environment` is injectable so tests never `setenv` (PROBLEMS.md).
  public static func defaultSessionsDirectory(
    environment: [String: String] = ProcessInfo.processInfo.environment,
    fileManager: FileManager = .default
  ) -> URL {
    let base: URL
    if let override = environment["CASETTE_CONFIG_DIR"], !override.isEmpty {

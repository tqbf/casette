...
| [plans/STAT.md](plans/STAT.md) | Statistics roadmap: human-friendly probability and descriptive-stat commands, starting with normal/binomial probability ranges and expanding to other distributions, plots, sampling, and formula bars. |
| [plans/PRELOAD.md](plans/PRELOAD.md) | Worker preload plan: human Python/Sage helper functions and constants installed before the symbol-table baseline so friendly lowerings and raw Sage share readable APIs without cluttering the user's Symbols list. |
...
| [plans/FORMULA-AUTOCOMPLETE.md](plans/FORMULA-AUTOCOMPLETE.md) | Formula autocomplete prototype: Numbers-style integral tokens, friendly-input IR, `wrt` support for explicit indefinite integrals, and the rule that UI preserves `#ROW` tape refs until `CompiledInput` expands them. |
| [plans/HELP-SYSTEM.md](plans/HELP-SYSTEM.md) | Native Help window for the Friendly Compiler mini-language, opened from the Help menu, with searchable command examples and generated Sage output. |
| [docs/COMPLETION-UI.md](docs/COMPLETION-UI.md) | Canonical completion UI contract: `ShellModel.draft` as source of truth, typed friendly IR, full-width hint lane layout, tape-reference preservation, the partial-edit round-trip rule, and extension rules. |
| [docs/COMPLETIONS-GUIDE.md](docs/COMPLETIONS-GUIDE.md) | The shipped completion vocabulary: every trigger/alias, accepted forms, chips, and Sage lowering for all 58 completions, plus the deliberately-deferred list. |
